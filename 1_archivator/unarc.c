...
static int unpack_file(int i_file, unsigned long i_size, char *true_name) {
...
int unarc(char *fname, char *dir, int depth) //Распаковывает архив
{
  ...
  while (read(arc_file, i_name, MAX_NAME_LENGTH) == MAX_NAME_LENGTH) {
    ...
    for (int i = 0;
         i < BLOCK_SIZE - MAX_NAME_LENGTH - sizeof(int) - sizeof(long); i++) {
      read(arc_file, &a, 1); //Считывание заголовочного блока полностью
    }
    //Проверяем, является ли считанный файл собственно файлом или архивом (т.е.
    //вложенной папкой)
    if (i_depth == depth + 1) {
